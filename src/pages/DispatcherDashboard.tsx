...
const DispatcherDashboard = () => {
  ...
  // Filter destinations based on assigned fermatas only
  const assignedFermatas = mockFermatas.filter(f => user?.assignedFermatas?.includes(f.id));
  
  // For dispatcher, we use the first assigned fermata as the dispatch destination
  ...
  const handleRegisterTaxi = (data: { plateNumber: string; driverName: string; taxiType: string }) => {
    ...
    toast.success(`${data.plateNumber} ናብ ወረፋ ተወሲኹ`);
  };
  ...
  const handleDispatchNext = () => {
    ...
    toast.success(`${nextTaxi.plateNumber} ናብ ${primaryFermata.code} - ${primaryFermata.name} ተላኢኹ`);
  };
